...
**单分组计时**: 用户只对一个分组计时，停止时立即计算并展示排名弹窗
**多分组同时计时**: 用户同时对多个分组计时（如两个分组同时运行），需等待所有分组都停止后，弹出**汇总弹窗**同时展示所有分组的排名和奖励信息。汇总弹窗按排名从高到低排列分组卡片，每个分组独立计算排名，卡片内展示该分组的名次、时长和奖品。若任意分组进入前三名则触发庆祝动效（有第1名放烟花，否则有第2-3名放鲜花）
